"""
CX Platforms – Ticket-volume forecast (PROD) - v10 (Final Combined)
    ...
    for cid in df['client_id'].unique():
        ...
        if 'TotalOrders' in X_tr_6.columns and not X_tr_6['TotalOrders'].isnull().all():
            ...
            if len(temp_df_simple) > 5:
                y_clean_simple, X_clean_simple = temp_df_simple['y'], temp_df_simple[simple_ols_features]
...
Traceback (most recent call last):
  File "c:\WFM_Scripting\Forecasting\Rpt_284_File.py", line 446, in <module>
    email_manager.handle_error("Ticket Forecasting Script Failure", exc, is_test=True)
  File "C:\WFM_Scripting\Automation\scripthelper.py", line 1170, in handle_error
    raise exception
  File "c:\WFM_Scripting\Forecasting\Rpt_284_File.py", line 287, in <module>
    ols_simple_model = sm.OLS(y_clean_simple, sm.add_constant(X_clean_simple)).fit()
                       ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
