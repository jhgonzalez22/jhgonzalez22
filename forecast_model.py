...
def safe_reg_metrics(y_true, y_pred):
    ...
    return dict(smape=smape(y[mask], p[mask]), mape=mape(y[mask], p[mask]),
...
def perform_statistical_tests(client_data: pd.DataFrame):
    ...
    correlations = {}
    potential_predictors = [col for col in client_data.columns if col.startswith('lag_') or col in ['sin_month', 'cos_month', 'TotalOrders']]
    for col in potential_predictors:
        predictor = client_data[col]
        mask = target.notna() & predictor.notna()
        if mask.sum() > 2:
            target_clean = target[mask].astype(float)
            predictor_clean = predictor[mask].astype(float)
            corr, p_val = pearsonr(target_clean, predictor_clean)
            correlations[col] = {'correlation': corr, 'p_value': p_val}
    results['correlations'] = correlations
    logger.info(f"\n--- Statistical Tests for: {results['client_id']} ---")
    logger.info(f"ADF P-Value: {results.get('adf_p_value', 'N/A'):.4f} (Stationary: {results.get('is_stationary', 'N/A')})")
...
def fetch_workload_drivers(bq: BigQueryManager) -> pd.DataFrame:
    sql = "SELECT MonthOfOrder, client_id, TotalOrders FROM tax_clnt_svcs.view_cx_nontax_platforms_workload_drivers WHERE MonthOfOrder >= '2023-01-01'"
    try:
        df = bq.run_gbq_sql(sql, return_dataframe=True)
        if df.empty: return pd.DataFrame(columns=["date", "client_id", "TotalOrders"])
        logger.info(f"✓ Drivers pulled: {len(df):,} rows")
        df['client_id'] = df['client_id'].replace({'FNC - CMS': 'FNC', 'FNC - Ports': 'FNC', 'Mercury Integrations': 'Mercury'})
        df = df.groupby(['MonthOfOrder', 'client_id'])['TotalOrders'].sum().reset_index()
...
def compute_client_driver_growth(drv_df: pd.DataFrame, cid: str, anchor: pd.Timestamp) -> float:
    ...
    if sub.empty or "TotalOrders" not in sub.columns: return np.nan
...
try:
    ...
    if tickets_df.empty: raise RuntimeError("BQ returned zero rows for tickets")
    ...
    forecasts, audit_rows, model_parameters, stat_test_results, all_modeling_data, xgb_imp_rows, all_model_summaries = [], [], [], [], [], [], []
    ...
    for cid in df['client_id'].unique():
        ...
        if len(ts) < (MAX_LAGS + VAL_LEN_6M + 1): continue

        feat = pd.DataFrame({'y': ts})
        feat = feat.join(client_df[['month', 'sin_month', 'cos_month', 'TotalOrders'] + [f'lag_{l}' for l in range(1, MAX_LAGS + 1)]])
        ...
        if len(feat) < (VAL_LEN_6M + 1): continue
        ...
        X_val_3, y_val_3 = feat.iloc[-VAL_LEN_3M:-VAL_LEN_3M+3].drop(columns='y'), feat.iloc[-VAL_LEN_3M:-VAL_LEN_3M+3]['y']
        ...
        good_cols = [c for c, g in imp_sorted if g > 0]
        ...
        if 'TotalOrders' in X_tr_6.columns and not X_tr_6['TotalOrders'].isnull().all():
            ...
            if len(temp_df_simple) > 5:
                ...
                all_model_summaries.append(f"--- Client: {cid}, Model: OLS_Simple ---\n{str(ols_simple_model.summary())}\n")
        ...
        def get_wlb_lags(block_idx):
            ...
            l12_pref, l12_fbk = lag12_ext.reindex(block_idx), feat['lag_12'].reindex(block_idx)
        ...
        if np.isfinite(r_drv):
            def wlbdrv_block_preds(idx, lam):
                wlb, sdrv = wlb_block_preds(idx), []
                for d in idx:
                    base = ts.get(d - pd.DateOffset(months=12), np.nan)
                    sdrv.append(base * (1.0 + r_drv) if np.isfinite(base) else np.nan)
                return lam*wlb + (1.0 - lam)*np.array(sdrv, dtype=float)
        ...
        else: logger.info(f"· {cid:<25} WLB+Drivers skipped")
        
        models_to_score = list(preds_3m.keys())
        for m in models_to_score:
            met3 = safe_reg_metrics(y_val_3.values, preds_3m.get(m, None))
            met6 = safe_reg_metrics(y_val_6.values, preds_6m.get(m, None))
            row = dict(client_id=cid, model=m, val_smape_3m=met3['smape'], val_smape_6m=met6['smape'],
                       MAPE_3m=met3['mape'], RMSE_3m=met3['rmse'], alpha_profile=DRV_ALPHA_STR)
            if m in extras: row.update(extras[m])
            audit_rows.append(row)

        if not preds_3m: logger.info(f"· {cid:<25} – no models could be fitted."); continue
        smapes_3 = {m: smape(y_val_3, p) for m, p in preds_3m.items() if np.isfinite(p).all()}
        if not smapes_3: continue
        best_model = min(smapes_3, key=smapes_3.get)
        
        for r in audit_rows[-len(models_to_score):]: r["winner_model"] = best_model
        ...
        def append_fx(d, v): forecasts.append(dict(fx_date=d.strftime('%Y-%m-%d'), client_id=cid, vol_type="tickets", fx_vol=safe_round(v), fx_id=fx_tag, fx_status="A", load_ts=STAMP))
        hist = deque(ts.tolist(), maxlen=max(24, MAX_LAGS))
        last_drv_val = client_drivers.iloc[-1].to_dict() if 'client_drivers' in locals() and client_drivers is not None and not client_drivers.empty else {}
        for d in future_idx:
            pred = np.nan
            if best_model in ['Ridge', 'Lasso', 'XGBoost']:
                row = {f'lag_{k}': hist[-k] for k in range(1, min(len(hist), MAX_LAGS) + 1)}
                row.update({ 'sin_month': np.sin(2*np.pi*d.month/12), 'cos_month': np.cos(2*np.pi*d.month/12), 'TotalOrders': last_drv_val.get('TotalOrders', np.nan) })
                xrow_df = pd.DataFrame([row])
                if best_model in ['Ridge', 'Lasso']:
                    pred = safe_expm1(models[best_model].predict(xrow_df[good_cols])[0])
                else: #XGBoost
                    pred = safe_expm1(models[best_model].predict(xrow_df[X_tr_6.columns])[0])
            elif best_model == 'OLS_Simple':
                row = {'month': d.month, 'TotalOrders': last_drv_val.get('TotalOrders', np.nan)}
                xrow_df = pd.DataFrame([row])[simple_ols_features]
                ...
                y3, y6, y12 = (hist[-3] if len(hist)>=3 else 0), (hist[-6] if len(hist)>=6 else 0), (hist[-12] if len(hist)>=12 else 0)
                pred = p['w_lag3']*y3 + p['w_lag6']*y6 + p['w_lag12']*y12
            elif best_model == 'SeasonalNaive3mGR':
                 base = hist[-12] if len(hist) >= 12 else hist[-1]
                 pred = base * (1 + (r_tgt if np.isfinite(r_tgt) else 0.0))
            elif best_model == 'WeightedLagBlendDrv':
                 p = extras['WeightedLagBlend']
                 y3, y6, y12 = (hist[-3] if len(hist)>=3 else 0), (hist[-6] if len(hist)>=6 else 0), (hist[-12] if len(hist)>=12 else 0)
                 wlb = p['w_lag3']*y3 + p['w_lag6']*y6 + p['w_lag12']*y12
                 sdrv_base = hist[-12] if len(hist) >= 12 else hist[-1]
                 sdrv = sdrv_base * (1.0 + r_drv)
    ...
    audit_df = pd.DataFrame(audit_rows).sort_values(["client_id","model"])
    
    logger.info(f"Pushing {len(fx_df):,} rows to {DEST_TABLE} …")
    if not bigquery_manager.import_data_to_bigquery(fx_df, DEST_TABLE, gbq_insert_action="append", auto_convert_df=True):
        raise RuntimeError("GBQ import failed")
    dedup_sql = f"UPDATE `{DEST_TABLE}` t SET t.fx_status = 'I' WHERE t.fx_status = 'A' AND EXISTS (SELECT 1 FROM `{DEST_TABLE}` s WHERE s.client_id = t.client_id AND s.vol_type = t.vol_type AND s.fx_date = t.fx_date AND s.load_ts > t.load_ts)"
